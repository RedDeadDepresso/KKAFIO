...
| `kkafio_coord_cache.json` | the coordinate directory | coordinate matching (`utils/chara_ops.py`, used by Archive/Delete Cards) | xxh3-128 digest of each coordinate card's clothes + accessory data (exact-match lookup against a chara card's outfits) |
